...
function render(){
  ...
  $("list").innerHTML=data.map(r=>'<article class="task"><input class="task-check" type="checkbox" data-done="'+esc(r.id)+'" '+(r.done?"checked":"")+' aria-label="Complete '+esc(r.title)+'"><div class="task-body"><div class="task-title '+(r.done?"done":"")+'">'+esc(r.title)+'</div>'+(r.note?'<div class="task-note">'+esc(r.note)+'</div>':'')+'<div class="task-meta"><span class="chip due">'+esc(r.done?"Completed":new Date(r.next).toLocaleString([],{dateStyle:"medium",timeStyle:"short"}))+'</span><span class="chip priority-'+(r.priority||"medium")+'">'+esc((r.priority||"medium").charAt(0).toUpperCase()+(r.priority||"medium").slice(1))+" Priority</span>"+(r.repeat!=="once"?'<span class="chip repeat">↻ every '+r.amount+" "+esc(units[r.repeat])+'</span>':"")+(r.labels||[]).map(l=>'<span class="chip label">#'+esc(l)+'</span>').join("")+'</div></div><div class="task-actions"><button data-edit="'+esc(r.id)+'">Edit</button>'+(!r.done?'<button data-calendar="'+esc(r.id)+'">Google Calendar</button><button data-snooze="'+esc(r.id)+'">Snooze 10m</button>':"")+'<button class="danger" data-delete="'+esc(r.id)+'">Delete</button></div></article>').join("");
}
async function load(){let qs=await getDocs(collection(db,"users",user.uid,"reminders"));reminders=qs.docs.map(snapshot=>({...snapshot.data(),id:snapshot.id}));if(!reminders.length){let legacy=await getDoc(doc(db,"users/"+user.uid));let old=legacy.exists()&&Array.isArray(legacy.data().reminders)?legacy.data().reminders:[];for(const r of old)await saveReminder(r);reminders=old}render()}function updateRepeatFields(){let show=$("repeat").value!=="once";$("amountWrap").style.display=show?"flex":"none";$("unitWrap").style.display=show?"flex":"none";$("amount").min=show?"1":"0";$("unitText").textContent="Every "+units[$("repeat").value]}function resetTaskForm(){editingId=null;$("form").reset();let current=new Date();$("date").value=current.toISOString().slice(0,10);$("time").value="09:00";$("formTitle").textContent="Add a task";$("saveTaskBtn").textContent="Save task";updateRepeatFields()}function openTaskForm(reminder=null){resetTaskForm();if(reminder){editingId=reminder.id;let due=new Date(reminder.next),local=new Date(due.getTime()-due.getTimezoneOffset()*60000).toISOString();$("title").value=reminder.title;$("date").value=local.slice(0,10);$("time").value=local.slice(11,16);$("repeat").value=reminder.repeat;$("amount").value=reminder.amount;$("priority").value=reminder.priority||"medium";$("labels").value=(reminder.labels||[]).join(", ");$("note").value=reminder.note||"";$("formTitle").textContent="Edit task";$("saveTaskBtn").textContent="Save changes"}updateRepeatFields();$("quickAdd").classList.remove("hidden");$("title").focus();window.scrollTo({top:0,behavior:"smooth"})}function add(){openTaskForm()}
...
$("signOutBtn").onclick=()=>signOut(auth);$("search").oninput=e=>{query=e.target.value;render()};$("sidebarAdd").onclick=add;$("headAdd").onclick=add;$("cancelAdd").onclick=()=>{resetTaskForm();$("quickAdd").classList.add("hidden")};document.querySelectorAll("[data-view]").forEach(b=>b.onclick=()=>{view=b.dataset.view;labelFilter="";document.querySelectorAll("[data-view]").forEach(x=>x.classList.toggle("active",x===b));render()});$("labelList").onclick=e=>{let b=e.target.closest("[data-label]");if(b){view="filters";labelFilter=b.dataset.label;render()}};$("repeat").onchange=updateRepeatFields;
$("form").onsubmit=async e=>{e.preventDefault();let repeat=$("repeat").value,d=new Date($("date").value+"T"+$("time").value),title=$("title").value.trim(),note=$("note").value.trim(),labels=$("labels").value.split(",").map(x=>x.trim().toLowerCase()).filter(Boolean);if(isNaN(d)){status("Please choose a valid date and time.");return}if(!title||title.length>200||note.length>2000||labels.length>20||labels.some(x=>x.length>50)){status("Please shorten the task, note, or labels. You can use up to 20 labels.");return}let existing=editingId?reminders.find(r=>r.id===editingId):null;if(editingId&&!existing){status("This task is no longer available. Refresh the page and try again.");return}let reminder={id:existing?existing.id:crypto.randomUUID(),title,note,labels,repeat,amount:repeat==="once"?0:Math.max(1,Number($("amount").value)||1),priority:$("priority").value,next:d.toISOString(),done:existing?existing.done:false};try{await saveReminder(reminder)}catch(error){status(saveErrorMessage(error));return}if(existing)reminders=reminders.map(r=>r.id===existing.id?reminder:r);else reminders.push(reminder);let updated=Boolean(existing);resetTaskForm();$("quickAdd").classList.add("hidden");render();status(updated?"Task updated.":"Task saved.")};
document.addEventListener("click",async e=>{let id=e.target.dataset.edit||e.target.dataset.calendar||e.target.dataset.snooze||e.target.dataset.delete||e.target.dataset.done;if(!id)return;let r=reminders.find(x=>x.id===id);if(!r)return;if(e.target.dataset.edit){openTaskForm(r);return}if(e.target.dataset.calendar){window.open(cal(r),"_blank","noopener,noreferrer");return}if(e.target.dataset.delete){reminders=reminders.filter(x=>x.id!==id);if(editingId===id){resetTaskForm();$("quickAdd").classList.add("hidden")}await deleteReminder(id)}else{if(e.target.dataset.snooze)r.next=new Date(Date.now()+600000).toISOString();if(e.target.dataset.done)r.done=e.target.checked;await saveReminder(r)}render()});onAuthStateChanged(auth,async u=>{user=u;if(u){$("loginGate").classList.add("hidden");$("app").classList.remove("hidden");$("quickAdd").classList.add("hidden");$("userEmail").textContent=u.email||"Signed in";$("sidebarUser").textContent=u.email||"Signed in";try{await load();requestAnimationFrame(()=>$("taskSection").scrollIntoView({behavior:"smooth",block:"start"}))}catch(e){status("Could not load reminders. Check Firestore rules.")}}else{if(idleTimer)clearTimeout(idleTimer);idleTimer=null;hideIdleWarning();$("loginGate").classList.remove("hidden");$("app").classList.add("hidden");reminders=[]}resetIdleTimer()});
